...
class OOMBatchRecordingModel(BatchRecordingModel):
    def __init__(self, maximum_batch_size):
        super().__init__()
        self.maximum_batch_size = int(maximum_batch_size)

    def __call__(self, input_ids):
        if input_ids.shape[0] > self.maximum_batch_size:
            raise torch.cuda.OutOfMemoryError("simulated rollout OOM")
        return super().__call__(input_ids)


def test_llada_rollout_uses_requested_microbatch_without_changing_results():
    ...
    full_model = BatchRecordingModel()
    chunked_model = BatchRecordingModel()

    full = complete_block_one_shot(
        candidates,
        full_model,
        mask_id=4,
        temperature=0.0,
    ...
    chunked = complete_block_one_shot(
        candidates,
        chunked_model,
        mask_id=4,
        temperature=0.0,
    ...
    assert full_model.batch_sizes == [6]
    assert chunked_model.batch_sizes == [2, 2, 2]
    torch.testing.assert_close(full, chunked)


def test_llada_rollout_batching_preserves_shared_rollout_noise():
    candidates = torch.tensor([[7, 4, 4]] * 6)
    full_model = BatchRecordingModel()
    chunked_model = BatchRecordingModel()
    ...
    full = complete_block_one_shot(
        ...
        gen_length=2,
        batch_sz=6,
        shared_noise_group_size=3,
        shared_noise_seed=29,
    )
    chunked = complete_block_one_shot(
        candidates,
        chunked_model,
...
def test_llada_rollout_full_batch_falls_back_without_dropping_rows():
    candidates = torch.tensor([[7, 4, 4]] * 6)
    model = OOMBatchRecordingModel(maximum_batch_size=3)
    stats = {}
    ...
    assert completed.shape == candidates.shape
    assert model.batch_sizes == [3, 3]
    assert stats == {
        "initial_batch_size": 6,
        "max_successful_batch_size": 3,
        "forward_batches": 2,
        "oom_retries": 1,
    }
